...
```
每个 TreeNode 包含一个值和两个指向其子节点的指针。在Go语言中，如果你想在一个函数中修改这个节点本身（例如删除节点或替换为另一个节点），仅仅传递一个指针（比如 *TreeNode）是不够的。因为这样只能修改节点的内容（如值或子节点的链接），而不能修改指向该节点的指针本身。

如果你传递了节点的地址（即节点指针的指针，也就是 **TreeNode），你就可以直接修改外部指向该节点的指针。这在你需要在函数内部“删除”或“完全替换”节点时非常有用。

示例
比如，如果你需要在二叉树中删除某个节点，只有通过双重指针，你才能确保在函数外部也反映出这个节点的删除（即将其设置为 nil）。

假设你有一个指向树的根节点的指针 root，你想删除这个根节点。如果你只是在函数内部将 root 设置为 nil，这只会影响函数内部的局部变量 root，而不会改变原始的树结构。但如果你传递了 root 的地址（&root），你可以在函数内将 *root = nil，这样就实际上将外部的 root 指针设置为了 nil。